...
#########################加代码#######################
    def box_label(self, box, label1="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        all = ['immature', 'mature']
        # 分离标签和置信度
        label_parts = label1.split(' ')
        label_name = label_parts[0]  # 获取标签部分（字符串）
        confidence = float(label_parts[1]) if len(label_parts) > 1 else 0.0  # 转换置信度为浮点数，默认为0.0
        # 根据提取的标签名确定标签索引
        if label_name == 'immature':
            label_index = 0  # 对应 'immature'
        elif label_name == 'mature':
            label_index = 1  # 对应 'mature'
        else:
            label_index = None  # 未知标签
        if label_index is not None:
            label = f"{all[label_index]} {confidence:.2f}"  # 显示标签和置信度
        else:
            label = "unknown"  # 默认值
    #####################################################
